...
It exists because one of the hardest things about the `now-playing` system is that there is not one universal source of truth for:
- current song
- current art
- current status
- what should be shown to the user

Instead, the authoritative path depends on the active playback mode.
...
Once that is known, the likely authority path becomes much clearer.
...
- art and current-song interpretation can differ by mode even when transport still exists underneath
...
When the active mode is local file playback, the first likely truths to verify are:
...
## What that means practically
When the active mode is AirPlay, the first likely truths to verify are:
...
## Common failure shapes
- missing or stale art because AirPlay metadata path is unhealthy
...
Current local-environment knowledge already says the AirPlay metadata path has local hardening/override behavior around `airplay-json`, watchdogs, and metadata readers.

That makes AirPlay one of the strongest cases where Brian’s local runtime reality matters.
...
A good current interpretation is:
- UPnP is its own branch, not just a generic stream case
- visible truth may combine:
...
## What that means practically
When the active mode is UPnP, the first likely truths to verify are:
...
- mixed metadata/art results where more than one authority path is plausible
...
- visible output may be shaped by a chain including:
...
In those cases, a good current interpretation is:
- fallback behavior becomes authoritative enough for presentation
- cache-backed current-art paths such as `/art/current.jpg` may become the practical visible truth
...
- local file wants local file/art truth
...
The next high-value companion pages are:
- `queue-wizard-internals.md`
- `controller-queue-interface.md`
- stronger route/file ownership proof for each authority path

## Current status

At the moment, this page gives the wiki one of the most important missing ideas:
