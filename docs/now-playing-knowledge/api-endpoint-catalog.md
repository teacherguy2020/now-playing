...
## Family: Seeburg wallbox integration

Owner:
- `src/routes/seeburg.routes.mjs`

Endpoints:
- `POST /integrations/seeburg/selection`
  - key-protected queue mutation
  - request body: `{ "number": 1 }` (or a raw JSON number)
...
- `GET /integrations/seeburg/playlist`
  ...
  - returns the current playlist count and its number-to-file mapping
...
```bash
curl -H 'x-track-key: YOUR_TRACK_KEY' \\
  -H 'Content-Type: application/json' \\
  -d '{"number":36,"dryRun":true}' \\
