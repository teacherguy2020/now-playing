...
app.get("/podcasts", async (req, res) => {
  ...
  // Optionally enrich with map counts if present
  const enriched = items.map(it => {
    let n = null;
    let lastBuilt = "";
    try {
...
// POST /podcasts/subscribe  { rss: "...", limit?: 200, download?: 10 }
app.post("/podcasts/subscribe", async (req, res) => {
    ...
    try {
        ...
        // ---- helper: keep spaces, remove filesystem-hostile chars (NO underscores)
        function safeFolderNameKeepSpaces(s, fallback = "Podcast") {
            ...
            t = t
                ...
                .replace(/[\/\\:*?"<>|]/g, "")            // path/Windows-hostile
        ...
        function safePlaylistCoverName(title, fallback = "Podcast") {
            // moOde playlist covers are files like "Pivot.jpg", "The Daily.jpg"
            // keep spaces, just strip characters that can't be in filenames
        ...
        async function pushPlaylistCoverToMoode(sub) {
            ...
            // Requires SSH key auth to moode@10.0.0.254 and sudo allowed for cp/chmod/chown
            const cmd = [
                "bash",
                "-lc",
                [
        ...
        // ---------- 1) Fetch RSS ONCE (best-effort) to learn title + show image
        ...
        if (!feedTitleRaw || feedTitleRaw === rss || !feedImageUrl) {
            try {
                ...
                // Light fetch: just enough items to get title/art
        ...
        // ---------- 2) Determine folder
        // IMPORTANT: if already subscribed, keep the existing folder basename (do not rename paths)
        let folder =
        ...
        // ---------- 3) Paths (Pi4 mount + moOde MPD prefix)
        ...
        // Human title for UI: prefer feed title, else prev title, else rss
        const title = feedTitleRaw || String(prev?.title || "").trim() || rss;

        // ---------- 4) Persist subscription object (NOW includes imageUrl)
        const sub = {
            ...prev,
            title,
            rss,
        ...
        // ---------- 4.5) Cover + playlist thumbnail deposit on moOde
        ...
        try {
            ...
            // writes: <sub.dir>/cover.jpg (Pi4 mount path, same file visible to moOde under /media/...)
        ...
        // ---------- 5) Single coherent stage:
        // Download N -> write map ONLY for downloaded/local -> write M3U from that map
        const work = { sync: null };

        try {
            logp("SYNC start", { limit, downloadCount });

            const r = await syncSubscriptionInitial({
                sub,
                limit,
                downloadCount
            });
...
async function ensureDir(p) {
  ...
  // If it has an extension, assume it’s a file path and mkdir the parent.
